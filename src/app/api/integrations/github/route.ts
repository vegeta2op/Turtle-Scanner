...
export async function POST(req: Request) {
  ...
  if (existing) {
    await prisma.integrationSettings.update({ where: { id: existing.id }, data: { accessTokenEnc: tokenEnc } });
  } else {
    await prisma.integrationSettings.create({ data: { provider: "GITHUB", userId: session.user.id, accessTokenEnc: tokenEnc } });
  }
...
export async function GET() {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) return new Response("Unauthorized", { status: 401 });
  const existing = await prisma.integrationSettings.findFirst({ where: { provider: "GITHUB", userId: session.user.id } });
