...
export async function POST(req: Request) {
  try {
    ...
    const existing = await prisma.integrationSettings.findFirst({ where: { provider: IntegrationProvider.GEMINI, userId: session.user.id } });
    if (existing) await prisma.integrationSettings.update({ where: { id: existing.id }, data: { accessTokenEnc: enc } });
    else await prisma.integrationSettings.create({ data: { provider: IntegrationProvider.GEMINI, userId: session.user.id, accessTokenEnc: enc } });
    return new Response(null, { status: 204 });
  } catch (e: any) {
    return new Response(e?.message || "Failed to save Gemini key", { status: 500 });
  }
}
