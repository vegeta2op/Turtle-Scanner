...
export async function POST(req: Request) {
  ...
  const existing = await prisma.integrationSettings.findFirst({ where: { provider: "SLACK", userId: session.user.id } });
  if (existing) {
    await prisma.integrationSettings.update({ where: { id: existing.id }, data: { accessTokenEnc: tokenEnc } });
  } else {
    await prisma.integrationSettings.create({ data: { provider: "SLACK", userId: session.user.id, accessTokenEnc: tokenEnc } });
  }
